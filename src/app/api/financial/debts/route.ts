...
export async function POST(req: NextRequest) {
  try {
    ...
    const created = await createDebt({
      ...
      balance: Math.max(0, parseFloat(body.balance) || 0),
      originalPrincipal: Math.max(0, body.originalPrincipal ? parseFloat(body.originalPrincipal) : parseFloat(body.balance) || 0),
      interestRate: Math.max(0, parseFloat(body.interestRate) || 0),
      minPayment: Math.max(0, parseFloat(body.minPayment) || 0),
      targetPayment: body.targetPayment ? Math.max(0, parseFloat(body.targetPayment)) : null,
